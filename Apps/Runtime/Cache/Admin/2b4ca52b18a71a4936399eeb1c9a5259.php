...
<html lang="zh-cn">
   ...
   function toEdit(id,pid){
       location.href='/index.php/Admin/GoodsCats/toEdit/?id='+id+"&parentId="+pid;     
   }
   function editName(obj){
	   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
	   $.post("/index.php/Admin/GoodsCats/editName",{id:$(obj).attr('dataId'),catName:obj.value},function(data,textStatus){
   ...
   function toggleIsShow(t,v){
	   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
	   $.post("/index.php/Admin/GoodsCats/editiIsShow",{id:v,isShow:t},function(data,textStatus){
			var json = WST.toJson(data);
			if(json.status=='1'){
   ...
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该分类吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   Plugins.closeWindow();
		   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
		   $.post("/index.php/Admin/GoodsCats/del",{id:id},function(data,textStatus){
				var json = WST.toJson(data);
				if(json.status=='1'){
					Plugins.setWaitTipsMsg({content:'操作成功',timeout:1000,callback:function(){
					    location.reload();
   ...
   <body class='wst-page'>
       ...
       <button type="button" class="btn btn-success glyphicon glyphicon-plus" onclick='javascript:toEdit(0)' style='float:right'>新增</button>
       <?php } ?>
       </div>
       <div class='wst-body'> 
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id='tr_0_<?php echo ($i); ?>' class="tr_0" isLoad='1'>
               ...
               <td>
               <?php if(in_array('spfl_01',$WST_STAFF['grant'])){ ?>
               <button type="button" class="btn btn-default glyphicon glyphicon-plus" onclick="javascript:toEdit(0,<?php echo ($vo['catId']); ?>)"">新增子分类</button>&nbsp;
               <?php } ?>
               <?php if(in_array('spfl_02',$WST_STAFF['grant'])){ ?>
               <button type="button" class="btn btn-default glyphicon glyphicon-pencil" onclick="javascript:toEdit(<?php echo ($vo['catId']); ?>,0)"">修改</button>&nbsp;
               <?php } ?>
               <?php if(in_array('spfl_03',$WST_STAFF['grant'])){ ?>
               <button type="button" class="btn btn-default glyphicon glyphicon-trash" onclick="javascript:del(<?php echo ($vo['catId']); ?>,0)"">刪除</button>
               <?php } ?>
               </td>
             </tr>
             <?php if($vo['childNum'] > 0): if(is_array($vo['child'])): $i2 = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i2 % 2 );++$i2;?><tr id='tr_0_<?php echo ($i); ?>_<?php echo ($i2); ?>' class="tr_0_<?php echo ($i); ?>" isLoad='1'>
	               ...
	               <td>
	               <?php if(in_array('spfl_01',$WST_STAFF['grant'])){ ?>
	               <button type="button" class="btn btn-default glyphicon glyphicon-plus" onclick="javascript:toEdit(0,<?php echo ($vo2['catId']); ?>)"">新增子分类</button>&nbsp;
	               <?php } ?>
	               <?php if(in_array('spfl_02',$WST_STAFF['grant'])){ ?>
	               <button type="button" class="btn btn-default glyphicon glyphicon-pencil" onclick="javascript:toEdit(<?php echo ($vo2['catId']); ?>,0)"">修改</button>&nbsp;
	               <?php } ?>
	               <?php if(in_array('spfl_03',$WST_STAFF['grant'])){ ?>
	               <button type="button" class="btn btn-default glyphicon glyphicon-trash" onclick="javascript:del(<?php echo ($vo2['catId']); ?>,0)"">刪除</button>
	               <?php } ?>
	               </td>
	             </tr>
                 <?php if($vo2['childNum'] > 0): if(is_array($vo2['child'])): $i3 = 0; $__LIST__ = $vo2['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo3): $mod = ($i3 % 2 );++$i3;?><tr id='tr_0_<?php echo ($i); ?>_<?php echo ($i2); ?>_<?php echo ($i3); ?>' class="tr_0_<?php echo ($i); ?>_<?php echo ($i2); ?>" isLoad='1'>
		               ...
		               <td>
		               <?php if(in_array('spfl_02',$WST_STAFF['grant'])){ ?>
		               <button type="button" class="btn btn-default glyphicon glyphicon-pencil" onclick="javascript:toEdit(<?php echo ($vo3['catId']); ?>,0)"">修改</button>&nbsp;
		               <?php } ?>
		               <?php if(in_array('spfl_03',$WST_STAFF['grant'])){ ?>
		               <button type="button" class="btn btn-default glyphicon glyphicon-trash" onclick="javascript:del(<?php echo ($vo3['catId']); ?>,0)"">刪除</button>
		               <?php } ?>
		               </td>
