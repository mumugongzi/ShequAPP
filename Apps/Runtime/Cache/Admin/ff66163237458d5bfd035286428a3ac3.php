...
<html lang="zh-cn">
   ...
   function query(){
	   ...
	   location.href="/index.php/Admin/Users/queryAccountByPage/?"+q.join('&');
   }
   function toggleStatus(t,v){
	   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
	   $.post("/index.php/Admin/Users/editUserStatus",{id:v,userStatus:t},function(data,textStatus){
   ...
   function toEdit(id){
	   Plugins.Modal({url:'/index.php/Admin/Users/toEditAccount/?id='+id,title:'编辑账号信息',width:500});
   }
   </script>
   <body class='wst-page'>
       <div class='wst-tbar'>
       会员账号：<input type='text' id='loginName2' name='loginName2' class='form-control wst-ipt-10' value='<?php echo ($loginName); ?>'/>
        ...
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               ...
               <td>
               <?php if(in_array('hyzh_04',$WST_STAFF['grant'])){ ?>
               <button type="button" class="btn btn-default glyphicon glyphicon-pencil" onclick="javascript:toEdit(<?php echo ($vo['userId']); ?>)"">修改</button>&nbsp;
               <?php } ?>
               </td>
