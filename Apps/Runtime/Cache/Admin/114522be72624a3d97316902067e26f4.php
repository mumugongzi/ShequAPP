...
<html lang="zh-cn">
   ...
   <script>
   function edit(id){
       location.href='/index.php/Admin/Areas/toEdit/?id='+id+'&parentId='+$('#parentId').val();     
   }
   function view(id){
       location.href='/index.php/Admin/Areas/index/?parentId='+id;     
   }
   function toggleIsShow(t,v){
	   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
	   $.post("/index.php/Admin/Areas/editiIsShow",{id:v,isShow:t},function(data,textStatus){
   ...
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该地区吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   Plugins.closeWindow();
		   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
		   $.post("/index.php/Admin/Areas/del",{id:id},function(data,textStatus){
					var json = WST.toJson(data);
					if(json.status=='1'){
						Plugins.setWaitTipsMsg({content:'操作成功',timeout:1000,callback:function(){
						   location.reload();
   ...
   <body class='wst-page'>
       ...
       <?php if($pArea['areaId'] !=0 ): ?><button type="button" class="btn glyphicon" onclick='javascript:location.href="/index.php/Admin/Areas/index/?parentId=<?php echo ($pArea['parentId']); ?>"' style='float:right;margin-left:5px;'>返回</button><?php endif; ?>
        ...
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               ...
               <td>
               <?php if($pArea['areaType'] < 1 ): ?><button type="button" class="btn btn-default glyphicon glyphicon-pencil" onclick="javascript:view(<?php echo ($vo['areaId']); ?>)"">查看</button>&nbsp;<?php endif; ?>
               <?php if(in_array('dqlb_02',$WST_STAFF['grant'])){ ?>
               <button type="button" class="btn btn-default glyphicon glyphicon-pencil" onclick="javascript:edit(<?php echo ($vo['areaId']); ?>)"">修改</button>&nbsp;
               <?php } ?>
               <?php if(in_array('dqlb_02',$WST_STAFF['grant'])){ ?>
