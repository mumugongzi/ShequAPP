...
<html lang="zh-cn">
   ...
   <script>
   function edit(id){
       location.href='/index.php/Admin/UserRanks/toEdit/?id='+id;     
   }
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该会员等级吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   Plugins.closeWindow();
		   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
		   $.post("/index.php/Admin/UserRanks/del",{id:id},function(data,textStatus){
        ...
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               ...
               <button type="button" class="btn btn-default glyphicon glyphicon-pencil" onclick="javascript:edit(<?php echo ($vo['rankId']); ?>)"">修改</button>&nbsp;
               <?php } ?>
