...
<div class='wst-page-content'>
   <div class='wst-tbar-group' style='text-align:left;'>
     <a href='javascript:toEditGoodsCat(0,0)'style='margin-right:15px;'><span class='add btn'></span>新增</a>
   </div>
   <table class='wst-list wst-form'>
   <thead>
   <tr>
     <th>名称</th>
     <th width='60'>排序号</th>
   ...
   <?php if(is_array($List)): $i = 0; $__LIST__ = $List;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr id='tr_0_<?php echo ($i); ?>' class="tr_0" isLoad='1'>
     <td>
     <span class='wst-tree-open' onclick='javascript:loadGoodsCatChildTree(this,<?php echo ($vo["catId"]); ?>,"tr_0_<?php echo ($i); ?>")'>&nbsp;</span>
     <input type='text' style='width:400px;height:22px;' value='<?php echo ($vo['catName']); ?>' dataId="<?php echo ($vo["catId"]); ?>" onblur='javascript:editGoodsCatName(this)'/>
     </td>
     <td><?php echo ($vo['catSort']); ?></td>
     <td>
     <?php if($vo['isShow']==0 ): ?><span class='wst-state_no'></span>
     <?php else: ?>
      <span class='wst-state_yes'></span><?php endif; ?>
     </td>
     <td>
     <a href="javascript:toEditGoodsCat(0,<?php echo ($vo['catId']); ?>)" class='add btn' title='新增'></a>
     <a href="javascript:toEditGoodsCat(<?php echo ($vo['catId']); ?>,0)" class='edit btn' title='编辑'>
     <a href="javascript:delGoodsCat(<?php echo ($vo['catId']); ?>,0)" class='del btn' title='删除'></a>&nbsp;
     </td>
   </tr>
   <?php if($vo['childNum'] > 0 ): if(is_array($vo['child'])): $i2 = 0; $__LIST__ = $vo['child'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo2): $mod = ($i2 % 2 );++$i2;?><tr id='tr_0_<?php echo ($i); ?>_<?php echo ($i2); ?>' class="tr_0_<?php echo ($i); ?>" isLoad='1'>
	     ...
	     <input type='text' style='width:400px;height:22px;' value='<?php echo ($vo2['catName']); ?>' dataId="<?php echo ($vo2["catId"]); ?>" onblur='javascript:editGoodsCatName(this)'/>
	     </td>
	     <td><?php echo ($vo2['catSort']); ?></td>
	     <td>
	     <?php if($vo2['isShow']==0 ): ?><span class='wst-state_no'></span>
	     <?php else: ?>
	      <span class='wst-state_yes'></span><?php endif; ?>
	     </td>
	     <td>
	     <a href="javascript:toEditGoodsCat(<?php echo ($vo2['catId']); ?>,0)" class='edit btn' title='编辑'>
