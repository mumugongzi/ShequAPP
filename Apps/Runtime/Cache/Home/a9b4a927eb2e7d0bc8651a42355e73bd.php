...
<style>
.<?php echo ($CONF['shopTitle']['fieldValue']); ?>-protocol {
    overflow: auto;
...
.<?php echo ($CONF['shopTitle']['fieldValue']); ?>-protocol h4{line-height: 30px;margin:0px;}
.<?php echo ($CONF['shopTitle']['fieldValue']); ?>-protocol p{line-height: 20px;margin:0px;}
.<?php echo ($CONF['shopTitle']['fieldValue']); ?>-protocol strong{margin-right:2px;}
</style>
</head>
<body>
<div class='<?php echo ($CONF['shopTitle']['fieldValue']); ?>-protocol'>
<h4><?php echo ($CONF['shopTitle']['fieldValue']); ?>注册协议</h4>
<p>本协议是您与<?php echo ($CONF['shopTitle']['fieldValue']); ?>（简称"本站"，网址：）所有者（以下简称为" "）之间就<?php echo ($CONF['shopTitle']['fieldValue']); ?>服务等相关事宜所订立的契约，请您仔细阅读本注册协议，您点击"同意并继续"按钮后，本协议即构成对双方有约束力的法律文件。</p>
<h4>第1条 本站服务条款的确认和接纳</h4>
<p><strong>1.1</strong>本站的各项电子服务的所有权和运作权归<?php echo ($CONF['shopTitle']['fieldValue']); ?>所有。用户同意所有注册协议条款并完成注册程序，才能成为本站的正式用户。用户确认：本协议条款是处理双方权利义务的契约，始终有效，法律另有强制性规定或双方另有特别约定的，依其规定。<p></p>
...
<p><strong>1.4</strong><?php echo ($CONF['shopTitle']['fieldValue']); ?>保留在中华人民共和国大陆地区法施行之法律允许的范围内独自决定拒绝服务、关闭用户账户、清除或编辑内容或取消订单的权利。</p>
<h4>第2条 本站服务</h4>
<p><strong>2.1</strong><?php echo ($CONF['shopTitle']['fieldValue']); ?>通过互联网依法为用户提供互联网信息等服务，用户在完全同意本协议及本站规定的情况下，方有权使用本站的相关服务。</p>
...
<h4>第3条 用户信息</h4>
<p><strong>3.1</strong>用户应自行诚信向本站提供注册资料，用户同意其提供的注册资料真实、准确、完整、合法有效，用户注册资料如有变动的，应及时更新其注册资料。如果用户提供的注册资料不合法、不真实、不准确、不详尽的，用户需承担因此引起的相应责任及后果，并且<?php echo ($CONF['shopTitle']['fieldValue']); ?>保留终止用户使用<?php echo ($CONF['shopTitle']['fieldValue']); ?>各项服务的权利。</p>
...
<p><strong>3.4</strong>用户同意，<?php echo ($CONF['shopTitle']['fieldValue']); ?>拥有通过邮件、短信电话等形式，向在本站注册、购物用户、收货人发送订单信息、促销活动等告知信息的权利。</p>
<p><strong>3.5</strong>用户不得将在本站注册获得的账户借给他人使用，否则用户应承担由此产生的全部责任，并与实际使用人承担连带责任。</p>
<p><strong>3.6</strong>用户同意，<?php echo ($CONF['shopTitle']['fieldValue']); ?>有权使用用户的注册信息、用户名、密码等信息，登录进入用户的注册账户，进行证据保全，包括但不限于公证、见证等。</p>
<h4>第4条 用户依法言行义务</h4>
...
<h4>第6条 商品信息</h4>
<p>本站上的商品价格、数量、是否有货等商品信息随时都有可能发生变动，本站不作特别通知。由于网站上商品信息的数量极其庞大，虽然本站会尽最大努力保证您所浏览商品信息的准确性，但由于众所周知的互联网技术因素等客观原因存在，本站网页显示的信息可能会有一定的滞后性或差错，对此情形您知悉并理解；<?php echo ($CONF['shopTitle']['fieldValue']); ?>欢迎纠错，并会视情况给予纠错者一定的奖励。</p>
...
<h4>第9条 交易争议处理</h4>
<p>您在淘宝平台交易过程中与其他用户发生争议的，您或其他用户中任何一方均有权选择以下途径解决：</p>
<p>（1）与争议相对方自主协商；</p>
<p>（2）使用<?php echo ($CONF['shopTitle']['fieldValue']); ?>网站提供的争议调处服务；</p>
...
<p><strong>10.2</strong>除非另有明确的书面说明,<?php echo ($CONF['shopTitle']['fieldValue']); ?>不对本站的运营及其包含在本网站上的信息、内容、材料、产品（包括软件）或服务作任何形式的、明示或默示的声明或担保（根据中华人民共和国法律另有规定的以外）。</p>
<p><?php echo ($CONF['shopTitle']['fieldValue']); ?>不担保本站所包含的或以其它方式通过本站提供给您的全部信息、内容、材料、产品（包括软件）和服务、其服务器或从本站发出的电子信件、信息没有病毒或其他有害成分。</p>
<p>如因不可抗力或其它本站无法控制的原因使本站销售系统崩溃或无法正常使用导致网上交易无法完成或丢失有关的信息、记录等，<?php echo ($CONF['shopTitle']['fieldValue']); ?>会合理地尽力协助处理善后事宜。</p>
<h4>第11条 协议更新及用户关注义务</h4>
<p>根据国家法律法规变化及网站运营需要，<?php echo ($CONF['shopTitle']['fieldValue']); ?>有权对本协议条款不时地进行修改，修改后的协议一旦被张贴在本站上即生效，并代替原来的协议。用户可随时登录查阅最新协议；用户有义务不时关注并阅读最新版的协议及网站公告。如用户不同意更新后的协议，可以且应立即停止接受<?php echo ($CONF['shopTitle']['fieldValue']); ?>网站依据本协议提供的服务；如用户继续使用本网站提供的服务的，即视为同意更新后的协议。<?php echo ($CONF['shopTitle']['fieldValue']); ?>建议您在使用本站之前阅读本协议及本站的公告。 如果本协议中任何一条被视为废止、无效或因任何理由不可执行，该条应视为可分的且并不影响任何其余条款的有效性和可执行性。</p>
<h4>附则</h4>
<p><strong>1</strong><?php echo ($CONF['shopTitle']['fieldValue']); ?>尊重用户和消费者的合法权利，本协议及本网站上发布的各类规则、声明等其他内容，均是为了更好的、更加便利的为用户和消费者提供服务。本站欢迎用户和社会各界提出意见和建议，<?php echo ($CONF['shopTitle']['fieldValue']); ?>将虚心接受并适时修改本协议及本站上的各类规则。</p>
...
<div id="btn"><input id="confirm" type="button" value="&nbsp;返回&nbsp;" onclick="javascript:history.go(-1)"/></div>
    </div>
