...
class UsersAction extends BaseAction {
    ...
    */
    	...
    	switch ($step) {
    		case 1:#第二步，验证身份
    			...
    			if ($info != false) {
    				...
    				if($info['userPhone']!='')$info['userPhone'] = $info['userPhone'];
	...
	public function getPhoneVerify(){
		...
		$USER['userPhone'] = $_POST['userPhoneNumber'];
		//dump($USER['userPhone']);
		...
		/*if (isset($_POST['userPhone'])) {
			$userPhone = isset($_POST['userPhone']);
		} else {
			$userPhone = session('findPass.userPhone');
		}*/
		$userPhone = isset($_POST['userPhoneNumber'])?$_POST['userPhoneNumber']:session('findPass.userPhone');
		$msg = "您正在重置登录密码，验证码为:".$phoneVerify."，请在30分钟内输入。【".$GLOBALS['CONFIG']['shopTitle']['fieldValue']."】";
	...
	public function checkPhoneVerify(){
		...
		dump(session('findPass.userPhone'));
		$value = session();
		dump($value);
		...
		if (!isset($_POST['phoneVerify'])) {
	            ...
	            return $this->json(100,"短信验证码正确");
	        }else{
	            //如果不成功
	            //echo "短信验证码发送失败，请联系客服";    
				return $this->json(400,"短信验证码错误");	
	       	}     
